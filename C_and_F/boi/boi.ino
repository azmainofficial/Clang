...
float temp = 0.0;
float Ftemp = 0.0;                       // variable to store the measured temperaturevariabele (float = floating point number)
...
void loop(void) { 
    ...
    Ftemp = (temp * 9/5) + 32;       // get the temperature of the first sensors in Celcius (lists start with 0 not 1)
    // temp = sensors.getTempFByIndex(0);   // get the temperature of the first sensors in Fahrenheit (lists start with 0 not 1)
    Serial.print("Temperature is: ");
    Serial.print(temp);
    Serial.print("C*   ");       // print message without a newline
    Serial.print(Ftemp);
    Serial.println("F*");                    // print the temperature with a newline

    delay(500);                            // pause 1000ms / 1 second
}